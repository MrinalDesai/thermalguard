...
def thermal_lut():
    """Thermal-camera style lookup table (Inferno when OpenCV is available)."""
    try:
        import cv2
...
def display_range(mats, min_span=3.0):
    """One adaptive temperature scale shared by all visible walls.

    A fixed 24-42C scale makes a 25-27C hotspot look almost identical.
    This preserves cross-wall comparability but expands the visible contrast.
    """
    vals = []
    for wall in WALLS:
        a = np.asarray(mats[wall], dtype=float)
    ...
    # Guarantee enough visual range so tiny sensor noise is not exaggerated
    # infinitely, while still making a real local rise clearly visible.
...
def temp_rgba(value, lo=VMIN, hi=VMAX):
    if not np.isfinite(value):
        return np.array([0.25, 0.25, 0.25, 1.0], dtype=np.float32)
    denom = max(float(hi) - float(lo), 1e-6)
    idx = int(np.clip(round((float(value) - lo) / denom * 255), 0, 255))
    rgb = LUT[idx].astype(np.float32) / 255.0
    return np.array([rgb[0], rgb[1], rgb[2], 1.0], dtype=np.float32)



def thermal_image(m, out_size=180):
    """Smooth thermography-style display from the real 4x4 wall matrix.

    DISPLAY ONLY:
      - ML/state logic still uses the original 4x4 measurements.
      - fully missing wall stays missing/grey.
      - isolated missing cells use the live wall mean only to keep the
    ...
    try:
        ...
        # Softens the 4x4 cell boundaries into a thermal-camera-like field.
        img = cv2.GaussianBlur(img, (0, 0), sigmaX=5.0, sigmaY=5.0)
        return img
    except Exception:
        # Lightweight bilinear fallback using only NumPy.
        x_old = np.arange(COLS, dtype=float)
        y_old = np.arange(ROWS, dtype=float)
        x_new = np.linspace(0, COLS - 1, out_size)
        y_new = np.linspace(0, ROWS - 1, out_size)

        tmp = np.empty((ROWS, out_size), dtype=np.float32)
        for r in range(ROWS):
            tmp[r] = np.interp(x_new, x_old, a[r])
...
def wall_face_colors(m, lo=VMIN, hi=VMAX):
    colors = []
    for r in range(ROWS):
        for c in range(COLS):
            rgba = temp_rgba(m[r, c], lo, hi)
...
def main():
    ...
    for i, wall in enumerate(WALLS):
        ...
        img = pg.ImageItem(np.full((180, 180), VMIN, dtype=float))
        img.setLookupTable(LUT)
        img.setLevels([VMIN, VMAX])
        img.setRect(-0.5, -0.5, 4.0, 4.0)
        p.addItem(img)
    ...
    def update_ui():
        ...
        # Shared adaptive thermal scale for all 4 walls.
        disp_lo, disp_hi = display_range(mats, min_span=3.0)
        ...
        if seq != last_seq:
            ...
            for wall in WALLS:
                ...
                # Smooth thermography reconstruction for DISPLAY ONLY.
                therm = thermal_image(m, out_size=180)
                image_items[wall].setImage(
                    therm,
                    autoLevels=False,
                    levels=(disp_lo, disp_hi),
                )
                for rr in range(ROWS):
                    for cc in range(COLS):
                        v = m[rr,cc]
                ...
                wall_meshes[wall].setMeshData(
                    vertexes=verts,
                    faces=faces,
                    faceColors=wall_face_colors(m, disp_lo, disp_hi),
                    smooth=False,
                )
        ...
        status.setText(
            ...
            f"   | thermal scale {disp_lo:.1f}-{disp_hi:.1f}C"
        )
