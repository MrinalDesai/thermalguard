...
class Shared:
    def __init__(self):
        ...
        self.frame = None
        self.ts = 0.0
        self.sim_label = "SIMULATED NORMAL"
...
def sim_thread(shared, inject_at=25):
    ...
    while True:
        n += 1
        if n == inject_at:
            sim.inject_hotspot("N", 1, 3, power=0.55)
            shared.sim_label = "SIMULATED LOCAL HOTSPOT"
            print(f"[sim] hotspot injected at frame {n}")
        if n == inject_at + 30:
            sim.inject_runaway("N", 1, 3)
            shared.sim_label = "SIMULATED THERMAL RUNAWAY"
...
def main():
    ...
    def update_ui():
        ...
        sim_tag = ""
        if args.source == "sim":
            sim_tag = f"\u26a0 {getattr(shared, 'sim_label', 'SIMULATED')}   "
        status.setText(
            sim_tag +
